...
/// Spectral delay with frequency-dependent delay times
///
/// Each frequency bin has an independent delay buffer, allowing for:
/// - Shimmer effects (high frequencies delay more)
/// - Inverse shimmer (low frequencies delay more)
/// - Spectral smearing and cascading delays
/// - Frequency-dependent echo patterns
///
/// # Performance
/// Uses SIMD for spectral operations. Latency is inherent to STFT block processing (~23ms @ 44.1kHz).
///
/// # Example
/// ```
...
/// delay.set_frequency_scale(1.0); // High freqs delay 2x as much
/// delay.set_mix(0.5);           // 50/50 dry/wet
///
/// let mut output = vec![0.0; 512];
    ...
    pub fn process(&mut self, output: &mut [f32], input: &[f32]) {
        ...
        self.stft.process(output, |spectrum| {
            ...
            for (bin_idx, bin) in spectrum.iter_mut().enumerate() {
                ...
                let feedback_signal = Complex::new(
                    dry.re + wet.re * feedback,
                    dry.im + wet.im * feedback,
                );
